...
@dp.table(name="earthquake_data_silver")
def earthquake_data_silver():
    ...
    df = (
        df.withColumn(
            "time", from_unixtime(col("time").cast("double") / 1000).cast("timestamp")
        )
...
@dp.table(name="earthquake_data_final")
def earthquake_data_final():
    df = spark.readStream.table("earthquake_data_silver")
